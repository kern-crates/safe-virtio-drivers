...
impl<H: Hal<QUEUE_SIZE>, T: Transport> VirtIOConsole<H, T> {
    ...
    pub fn info(&self) -> VirtIoResult<ConsoleInfo> {
        ...
        let columns = self.config_space.cols.read_u16(io_region)?;
        let rows = self.config_space.rows.read_u16(io_region)?;
        let max_ports = self.config_space.max_nr_ports.read_u32(io_region)?;
        Ok(ConsoleInfo {
            rows,
            columns,
            max_ports,
